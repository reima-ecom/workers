 ...
 */
const getSubscriber = (fetch, SHOPIFY_BASIC_AUTH) => {
  if (!fetch || !SHOPIFY_BASIC_AUTH) {
  ...
  const request = async (urlPath, method, data) => {
    const resp = await fetch(`https://reima-us.myshopify.com${urlPath}`, {
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `Basic ${SHOPIFY_BASIC_AUTH}`,
      },
